...
while true; do
    if _socket_ready && _setup_display_auth; then
        ...
        # Запускаем GUI напрямую (минуя fallback в main.py). Если Tkinter упадёт
        # (например, сессия завершилась) — поднимем монитор и попробуем снова.
        python3 -c "from gui import launch; launch()"
        log "GUI завершился (код $?) — возвращаемся к фоновому монитору"
        python3 /app/usb_monitor.py &
        MONITOR_PID=$!
    fi
    sleep 5
