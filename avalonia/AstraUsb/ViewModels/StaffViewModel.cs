...
///
/// Камера сама сообщает номер сотрудника в именах записей, и станция заводит
/// по нему карточку. Но имя за неё никто не впишет, поэтому оператору нужно
/// место, где номер превращается в человека с отделом и телефоном.
/// </summary>
    ...
    public void Reload()
    {
        ...
        try
        {
            ...
            Hint = Employees.Count == 0
                ? "сотрудников пока нет; они появятся сами, когда камера сообщит свой номер"
                : "";
        }
        catch (Exception e)
        {
