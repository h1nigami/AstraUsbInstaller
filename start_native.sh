#!/bin/bash
# Запуск станции: дожидается графической сессии и запускает GUI.
...
# флешек). Скрипт ожидает доступности графической сессии:
#
...
while true; do
    if _detect_display && _setup_display_auth; then
        ...
        # systemd видит код выхода GUI и перезапускает его только при ошибке.
        exec python3 -c "from gui import launch; launch()"
    fi
    sleep 5
done
