#!/bin/bash
# Нативный запуск БЕЗ Docker: дожидается графической сессии и запускает GUI.
# Мониторинг USB отдельно не запускается — он работает внутри GUI (gui.py
# сам поднимает monitor_usb в фоновом потоке). Пока пользователь не вошёл
# в сессию, приложение просто ждёт.
#
# Запускается XDG autostart (.desktop) через sudo от root (root нужен приложению для mount/umount
...
log "Ждём доступности X11 для запуска GUI (проверка каждые 5 секунд)"
while true; do
    if _detect_display && _setup_display_auth; then
        log "X11 доступен (DISPLAY=$DISPLAY) — запускаем GUI"
        # Запускаем GUI напрямую (минуя fallback в main.py), чтобы ошибка
        # подключения к X дала ненулевой код, а не тихий headless-режим.
        ...
        if [ "$rc" -eq 0 ]; then
            ...
            log "Штатный выход из GUI по паролю — сервис останавливается"
            exit 0
        fi
        # Ненулевой код = аварийное завершение (например, пропала X-сессия):
        # ждём и пробуем поднять GUI снова.
        log "Аварийное завершение GUI — повторный запуск через 5 секунд"
    fi
    sleep 5
done
