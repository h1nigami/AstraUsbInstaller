...
# Логика выключения второй версии живёт в remove_station_v2.sh: её же зовёт
# кнопка диагностики в интерфейсе, а описана она должна быть один раз.
SUDO="$SUDO" ASTRA_ROOT="${ASTRA_ROOT:-}" bash "$SRC_DIR/remove_station_v2.sh"
...
$SUDO cp "$SRC_DIR/main.py" "$SRC_DIR/gui.py" "$SRC_DIR/usb_monitor.py" \
         "$SRC_DIR/updater.py" "$SRC_DIR/start_native.sh" \
         "$SRC_DIR/remove_station_v2.sh" "$APP_DIR/"
